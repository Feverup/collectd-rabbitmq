...
def read(input_data=None):
  ...
  for host in PLUGIN_CONFIG['host'].split(',') :
    ...
    #collectd.warning("Dispatching %s %s %s\n\t%s " % ( cluster_name , 'rabbitmq' , 'rabbit_overview' , values ) )
   ...
   #     collectd.warning("     = %s" % values)
        ...
        #  collectd.warning("Dispatching %s %s %s\n\t%s " % ( node['name'].split('@')[1] , 'rabbitmq_node' , 'rabbit_node' , values ) )
          dispatch_values(values, node['name'].split('@')[1],
                        'rabbitmq_node', None, 'rabbit_node')
          dispatch_values(values, node['name'].split('@')[1],
                        'rabbitmq_node', None, 'rabbit_node_new')
        values = mefiltras( map( node.get , NODE_IO ) )
        if values.count(None) < 1 :
        #  collectd.warning("Dispatching %s %s %s\n\t%s " % ( node['name'].split('@')[1] , 'rabbitmq_node' , 'rabbit_io' , values ) )
          dispatch_values(values, node['name'].split('@')[1],
                        'rabbitmq_node', None, 'rabbit_io')
          dispatch_values(values, node['name'].split('@')[1],
                        'rabbitmq_node', None, 'rabbit_io_new')
    ...
    for vhost in get_info("%s/vhosts" % (base_url)):
        ...
        if vhost.has_key( 'message_stats' ) :
            ...
            #collectd.warning("Dispatching %s %s %s\n\t%s " % ( cluster_name , vhost_safename , 'rabbit_vhost' , values ) )
            #dispatch_values(values, cluster_name, vhost_safename, None, 'rabbit_vhost')
        #    collectd.warning("Dispatching %s %s-%s %s\n\t%s " % ( cluster_name , 'vhost' , vhost['name'].replace('/', 'default'), 'rabbit_vhost' , values ) )
            ...
            #if values[-1] is None :
            #    values[-1] = 0
        #    collectd.warning("Dispatching %s %s-%s %s\n\t%s " % ( cluster_name , 'messages' , vhost['name'].replace('/', 'default') , 'rabbit_messages' , values ) )
        ...
        for queue in get_info("%s/queues/%s" % (base_url, vhost_name)):
            ...
            if not want_to_ignore("queue", queue_name) and queue['durable'] :
                ...
                #    collectd.warning("Dispatching %s %s-%s %s\n\t%s " % ( cluster_name , 'queue' , queue_data['name'] , 'rabbit_queue' , values ) )
        ...
        for exchange in get_info("%s/exchanges/%s" % (base_url,
            ...
            if exchange_name and exchange['durable'] :
                ...
                if exchange_data.has_key('message_stats'):
                    ...
                    if exchange_data['name'] == 'photos' :
                        collectd.warning("Dispatching %s %s-%s %s\n\t%s " % ( cluster_name , 'exchange' , exchange_data['name'], 'rabbit_exchange' , values ) )
